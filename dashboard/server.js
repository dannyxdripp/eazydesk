...
#toast-container{position:fixed;bottom:20px;right:20px;display:none;flex-direction:column;gap:12px;z-index:9999;max-width:420px}.toast{display:flex;align-items:flex-start;gap:12px;padding:14px 16px;border-radius:12px;border:1px solid;background:rgba(255,255,255,.05);backdrop-filter:blur(8px);box-shadow:0 8px 32px rgba(0,0,0,.3);animation:toastSlideIn 300ms ease forwards;opacity:1}.toast.toast-ok{border-color:rgba(87,242,135,.35);background:rgba(87,242,135,.08)}.toast.toast-ok .toast-icon{color:#57f287}.toast.toast-danger{border-color:rgba(237,66,69,.42);background:rgba(237,66,69,.08)}.toast.toast-danger .toast-icon{color:#ed4245}.toast.toast-warn{border-color:rgba(254,231,92,.38);background:rgba(254,231,92,.08)}.toast.toast-warn .toast-icon{color:#fee71c}.toast-icon{font-size:18px;font-weight:900;flex:0 0 auto;line-height:1}.toast-content{flex:1 1 auto}.toast-title{font-size:14px;font-weight:700;display:block;margin:0 0 2px}.toast-message{font-size:13px;color:var(--muted);margin:0}.toast-close{background:none;border:none;color:var(--muted);cursor:pointer;padding:2px 4px;font-size:18px;line-height:1;transition:color 200ms;flex:0 0 auto}.toast-close:hover{color:var(--text)}@keyframes toastSlideIn{from{transform:translateX(420px);opacity:0}to{transform:translateX(0);opacity:1}}@keyframes toastSlideOut{from{transform:translateX(0);opacity:1}to{transform:translateX(420px);opacity:0}}
...
const app=document.getElementById('app'),notice=document.getElementById('notice'),auth=document.getElementById('auth'),authDiscord=document.getElementById('authDiscord'),authToken=document.getElementById('authToken'),authMsg=document.getElementById('authMsg');
 ...
 const inferTheme=()=>{try{const saved=String(localStorage.getItem(themeKey)||'dark').toLowerCase();if(saved==='hacker'&&!isHackerUnlocked())return 'dark';return ['dark','light','ocean','sunset','hacker'].includes(saved)?saved:'dark'}catch{return 'dark'}};
