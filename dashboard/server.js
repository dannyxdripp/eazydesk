...
function decodeDiscordTokenSubjectId(token) {
...
function getKnownApplicationIds() {
    return [...new Set([
...
function validateCustomBotTargetGuildId(guildId, customBot = {}) {
    const id = String(guildId || '').trim();
    if (!/^\d{17,20}$/.test(id)) return 'Invalid server ID.';

    const appId = String(customBot?.appId || '').trim();
    if (appId && appId === id) {
        return 'The target Server ID and App ID are the same. Paste the Discord server/guild ID into Server ID, and the application/client ID into App ID.';
    }

    const tokenSubjectId = decodeDiscordTokenSubjectId(customBot?.token || '');
    if (tokenSubjectId && tokenSubjectId === id) {
        return 'That Server ID matches the custom bot account ID from the token. Paste the Discord server/guild ID instead.';
    }

    if (getKnownApplicationIds().includes(id)) {
        return 'That looks like a bot application/client ID, not a Discord server ID. Paste the server/guild ID from Discord.';
    }
...
function streamFileToResponse(res, filePath) {
    const stream = fs.createReadStream(filePath);
    stream.on('error', error => {
        console.error('[Dashboard] File stream error:', error?.message || error);
        if (!res.writableEnded) {
            res.writeHead(500, { 'Content-Type': 'text/plain; charset=utf-8', 'Cache-Control': 'no-store' });
            res.end('File stream failed');
        } else {
...
<body class="home">
    ...
    <section id="pricing" class="section reveal">
      <div class="container">
        ...
        <div class="pricing">
          ...
          <div class="price-card"><h3>Enterprise</h3><p>Custom branded bot runtime and guided setup.</p><div class="price">Custom</div><ul><li>Custom bot instance</li><li>Developer Portal guidance</li><li>Runtime monitoring</li></ul><a class="btn btn-ghost" href="/upgrade">Talk Enterprise</a></div>
        </div>
      </div>
    </section>
  ...
  <footer class="footer">
    <div class="container footer-grid">
      ...
      <div><h4>Product</h4><a href="/pricing">Pricing</a><a href="/upgrade">Upgrade</a><a href="/dashboard">Dashboard</a></div>
...
function createSetupHtml(req = null) {
        ...
        <div class="setup-grid">
          <div class="card setup-panel">
            ...
            <section class="setup-stage" data-step="2">
              ...
              <select id="updatesChannelId"></select>
              <div class="setup-inline" style="margin-top:10px">
                <button class="btn" type="button" data-create-kind="updates">Create a channel for me</button>
              </div>
              <div class="setup-actions">
    ...
    const script = `
      ...
      const updatesChannelId=document.getElementById('updatesChannelId');
      ...
      function esc(s){return String(s||'').replace(/[&<>\"']/g,m=>({ '&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',\"'\":'&#39;' }[m]))}
      const setupSelectPlaceholders={guildSelect:'Select a server',parentCategoryId:'Not set',appealsChannelId:'Not set',transcriptsChannelId:'Not set',updatesChannelId:'Not set',managerRoleId:'Optional',highEscalationRoleId:'Optional',immediateEscalationRoleId:'Optional'};
      ...
      function refreshAllSetupSelects(){for(const el of [guildSelect,parentCategoryId,appealsChannelId,transcriptsChannelId,updatesChannelId,managerRoleId,highEscalationRoleId,immediateEscalationRoleId])refreshSetupSelect(el,setupSelectPlaceholders[el&&el.id]||'Select')}
      const csrfToken=${JSON.stringify(getDashboardSessionCsrfToken(req) || '')};
      async function api(path,opt){const headers={...(opt&&opt.headers||{})};if(csrfToken&&String((opt&&opt.method)||'GET').toUpperCase()!=='GET')headers['x-csrf-token']=csrfToken;const r=await fetch(path,{credentials:'include',...(opt||{}),headers});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||('Request failed '+r.status));return d}
      ...
      function configPayload(){return{guildId:guildSelect.value,parentCategoryId:parentCategoryId.value||null,appealsChannelId:appealsChannelId.value||null,transcriptsChannelId:transcriptsChannelId.value||null,updatesChannelId:updatesChannelId.value||null,managerRoleId:managerRoleId.value||null,escalationRoles:{high:highEscalationRoleId.value||null,immediate:immediateEscalationRoleId.value||null},rolePermanence:!!rolePermanence.checked,tutorialEnabled:!!tutorialEnabled.checked,setup:{step:currentStep}}}
      ...
      function renderSummary(){if(!summary)return;summary.innerHTML=''+
        ...
        '<div class=\"item\"><div><strong>Updates Channel</strong><div class=\"muted\">'+esc(readLabel(updatesChannelId))+'</div></div></div>'+
      ...
      async function createChannel(kind){const gid=guildSelect.value;if(!gid)throw new Error('Pick a guild first.');const defaults={category:'Tickets',feedback:'ticket-feedback',transcripts:'ticket-transcripts',updates:'ticket-updates'};const label=kind==='category'?'category':'channel';const name=prompt('Name for the new '+label+':',defaults[kind]||'tickets');if(name===null)return false;const trimmed=String(name||'').trim();if(!trimmed)throw new Error('A name is required.');await saveConfig({setup:{step:2}});const result=await api('/api/setup/create-channel',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({guildId:gid,kind,name:trimmed,parentCategoryId:parentCategoryId.value||null})});await loadCatalogs();if(kind==='category')parentCategoryId.value=result.channel.id;else if(kind==='feedback')appealsChannelId.value=result.channel.id;else if(kind==='transcripts')transcriptsChannelId.value=result.channel.id;else if(kind==='updates')updatesChannelId.value=result.channel.id;refreshAllSetupSelects();renderSummary();return true}
      ...
      [guildSelect,parentCategoryId,appealsChannelId,transcriptsChannelId,updatesChannelId,managerRoleId,highEscalationRoleId,immediateEscalationRoleId,rolePermanence,tutorialEnabled].forEach(el=>{if(el)el.onchange=renderSummary});
...
function normalizeOpeningQuestionsInput(input, max = 5) {
    ...
    return rawList
        .map((entry) => {
            const raw = typeof entry === 'string' ? { label: entry } : (entry && typeof entry === 'object' ? entry : {});
            const label = String(raw.label || raw.question || raw.title || '').trim().slice(0, 45);
            if (!label) return null;
            const style = String(raw.style || raw.type || '').trim().toLowerCase() === 'short' ? 'short' : 'paragraph';
            return {
                label,
                placeholder: String(raw.placeholder || '').trim().slice(0, 100),
                required: raw.required === undefined ? true : Boolean(raw.required),
                style,
                maxLength: Math.max(50, Math.min(1024, Number(raw.maxLength || (style === 'short' ? 240 : 1024))))
            };
        })
        .filter(Boolean)
        .slice(0, Math.max(1, Math.min(5, Number(max) || 5)));
}
...
async function handleApi(req, res, url, client, customBotManager = null) {
    ...
    if (method === 'GET' && pathname === '/api/controller/guilds') {
        ...
        const guilds = [
            ...
            : []),
            ...Object.keys(activeStorage.aiGuildAccess || {})
                .filter(id => /^\d{17,20}$/.test(id) && !seenGuildIds.has(id))
                .map(id => {
                    const cfg = typeof ticketStore.getGuildConfig === 'function' ? ticketStore.getGuildConfig(id, activeStorage) : {};
                    const ai = getGuildAiUiState(id, activeStorage);
                    return {
    ...
    if (method === 'POST' && pathname === '/api/guild-config') {
        ...
        const existingSetup = existingConfig?.setup && typeof existingConfig.setup === 'object'
            ...
            : {};
        const next = {};
        for (const key of ['parentCategoryId', 'appealsChannelId', 'transcriptsChannelId', 'updatesChannelId', 'managerRoleId']) {
        ...
        if (body.branding && typeof body.branding === 'object') {
            ...
            const identityChangeRequested = Object.prototype.hasOwnProperty.call(body.branding, 'botName') ||
                Object.prototype.hasOwnProperty.call(body.branding, 'avatarUrl');
            if (identityChangeRequested && !plan.isCustom) {
                sendJson(res, 403, { error: 'Bot name and avatar branding requires the Custom plan.' });
                return true;
            }
        ...
        if (Object.prototype.hasOwnProperty.call(body, 'setupComplete')) {
            ...
            const resolvedUpdatesChannelId = String(next.updatesChannelId || existingConfig?.updatesChannelId || '').trim();
            if (setupComplete && !/^\d{17,20}$/.test(resolvedUpdatesChannelId)) {
                sendJson(res, 400, { error: 'Choose an updates channel before completing setup.' });
                return true;
            }
            next.setup = {
                ...existingSetup,
    ...
    if (method === 'POST' && pathname === '/api/setup/create-channel') {
        ...
        if (!['category', 'feedback', 'transcripts', 'updates'].includes(kind)) {
    ...
    if (method === 'POST' && pathname === '/api/owner/guild-ai') {
        ...
        if (action === 'start-trial') {
            ...
            const plan = ['plus', 'pro', 'custom', 'premium'].includes(String(body.plan || '').trim().toLowerCase())
                ...
                : 'plus';
            if (plan === 'custom') {
                const customBotInput = body.customBot && typeof body.customBot === 'object' ? body.customBot : current.customBot;
                const targetError = validateCustomBotTargetGuildId(guildId, customBotInput);
                if (targetError) {
        ...
        if ((action === 'set-premium' || action === 'set-plan') && ['custom', 'custom_trial'].includes(String(updated?.plan || ''))) {
            const cfg = typeof ticketStore.getGuildConfig === 'function' ? ticketStore.getGuildConfig(guildId, activeStorage) : {};
            if (!cfg || !Object.keys(cfg).length) {
                ticketStore.setGuildConfig(guildId, {
                    setup: {
                        completed: false,
                        createdAt: new Date().toISOString(),
                        source: 'owner-custom-only'
                    },
                    ticketTypes: [],
                    supportTeams: [],
                    tags: [],
    ...
    if (method === 'POST' && pathname === '/api/ticket-type/upsert') {
        ...
        const allowAttachments = body.allowAttachments !== false;
        const questionLimit = plan.hasAccess ? (allowAttachments ? 4 : 5) : 1;
        const openQuestions = normalizeOpeningQuestionsInput(body.openQuestions, questionLimit);
        const nextType = {
            name,
...
function getAllowedDashboardPages(access = {}) {
    ...
    pages.add('/dashboard/pricing');
    pages.add('/dashboard/upgrade');
    if (access?.canFullDashboard || access?.isOwner) {
        ['/overview', '/settings', '/availability', '/commands/ticket-types', '/panels', '/commands/tag', '/tickets', '/transcripts', '/commands/feedback', '/statistics', '/embed-editor', '/tutorials', '/dashboard/pricing', '/dashboard/upgrade', '/setup', '/controller', '/custom-bots', '/privacy', '/terms'].forEach(page => pages.add(page));
        return pages;
    }
...
function pageDescriptionForPath(path) {
    const map = {
        ...
        '/dashboard/pricing': 'Compare plans and see what is available for this server.',
        '/dashboard/upgrade': 'Upgrade to Plus or Pro as one-time purchases, or contact sales for Enterprise.',
...
function renderDashboardSidebar(currentPath) {
    ...
    const settingsLinks = [
        ...
        ['/dashboard/pricing', 'Premium', 'settings', 'pricing']
    ];
...
.list-btn.active{border-color:rgba(56,189,248,.34);background:linear-gradient(180deg,rgba(56,189,248,.14),rgba(255,255,255,.03));box-shadow:inset 0 1px 0 rgba(255,255,255,.10)}
 ...
 textarea{min-height:110px}
  ...
  :root{
    --dash-bg:#070816;
    --dash-bg-2:#0a1024;
    --dash-panel:rgba(15,18,36,.84);
    --dash-panel-2:rgba(20,24,48,.74);
    --dash-panel-3:rgba(26,31,62,.82);
    --dash-border:rgba(164,174,255,.16);
    --dash-border-strong:rgba(139,92,246,.46);
    --dash-purple:#8b5cf6;
    --dash-blue:#3b82f6;
    --dash-cyan:#22d3ee;
    --dash-green:#22c55e;
    ...
    --dash-red:#ef4444;
    --dash-text:#f7f8ff;
    --dash-muted:rgba(226,232,255,.70);
    --dash-muted-2:rgba(226,232,255,.50);
    --dash-shadow:0 22px 70px rgba(0,0,0,.34);
    --dash-glow:0 0 36px rgba(139,92,246,.14);
    --ac:var(--dash-purple);
    --ac-soft:var(--dash-cyan);
    --tx:var(--dash-text);
    --mt:var(--dash-muted);
    --bd:var(--dash-border);
  }
  body[data-theme="light"]{
    --dash-bg:#f6f8ff;--dash-bg-2:#edf2ff;--dash-panel:rgba(255,255,255,.90);--dash-panel-2:rgba(248,250,255,.92);--dash-panel-3:#fff;--dash-border:rgba(58,72,120,.16);--dash-border-strong:rgba(99,102,241,.40);--dash-text:#111827;--dash-muted:rgba(31,41,55,.70);--dash-muted-2:rgba(31,41,55,.52);--dash-shadow:0 18px 44px rgba(30,41,59,.14);--dash-glow:0 0 32px rgba(99,102,241,.12)
  }
  body[data-theme="ocean"]{--dash-bg:#04131f;--dash-bg-2:#062437;--dash-panel:rgba(7,28,45,.86);--dash-panel-2:rgba(9,39,59,.76);--dash-panel-3:rgba(13,54,75,.82);--dash-border:rgba(125,211,252,.18);--dash-border-strong:rgba(34,211,238,.46);--dash-purple:#38bdf8;--dash-blue:#2563eb;--dash-cyan:#2dd4bf;--dash-glow:0 0 34px rgba(34,211,238,.15)}
  body[data-theme="sunset"]{--dash-bg:#160b1c;--dash-bg-2:#251126;--dash-panel:rgba(44,20,45,.86);--dash-panel-2:rgba(57,24,47,.76);--dash-panel-3:rgba(70,30,55,.82);--dash-border:rgba(251,146,60,.18);--dash-border-strong:rgba(251,113,133,.46);--dash-purple:#fb7185;--dash-blue:#8b5cf6;--dash-cyan:#f59e0b;--dash-glow:0 0 34px rgba(251,113,133,.15)}
  body[data-theme="diamond"]{--dash-bg:#061018;--dash-bg-2:#0b1724;--dash-panel:rgba(8,21,33,.86);--dash-panel-2:rgba(12,31,45,.78);--dash-panel-3:rgba(17,42,58,.84);--dash-border:rgba(165,243,252,.20);--dash-border-strong:rgba(103,232,249,.48);--dash-purple:#67e8f9;--dash-blue:#60a5fa;--dash-cyan:#d9f99d;--dash-glow:0 0 38px rgba(103,232,249,.14)}
  body[data-theme="dark"],body:not([data-theme]),body[data-theme="diamond"],body[data-theme="ocean"],body[data-theme="sunset"]{
    color:var(--dash-text);
    background:
      radial-gradient(900px 520px at 20% -8%,color-mix(in srgb,var(--dash-purple) 22%,transparent),transparent 62%),
      radial-gradient(760px 460px at 98% 8%,color-mix(in srgb,var(--dash-cyan) 14%,transparent),transparent 58%),
      radial-gradient(780px 520px at 80% 110%,rgba(34,197,94,.08),transparent 60%),
      linear-gradient(145deg,var(--dash-bg),#080b1d 52%,var(--dash-bg-2));
  }
  .layout{display:grid!important;grid-template-columns:280px minmax(0,1fr);min-height:100vh}
  .app-sidebar{position:sticky;top:0;height:100vh;display:flex;flex-direction:column;gap:18px;padding:22px 16px;background:linear-gradient(180deg,color-mix(in srgb,var(--dash-panel) 96%,#050611),color-mix(in srgb,var(--dash-bg) 92%,transparent));border-right:1px solid var(--dash-border);box-shadow:18px 0 70px rgba(0,0,0,.28);overflow:auto;z-index:20}
  ...
  .side-link.active{color:#fff;background:linear-gradient(135deg,color-mix(in srgb,var(--dash-purple) 28%,transparent),color-mix(in srgb,var(--dash-blue) 15%,transparent));border-color:var(--dash-border-strong);box-shadow:0 0 0 1px rgba(34,211,238,.08),0 14px 34px color-mix(in srgb,var(--dash-purple) 18%,transparent)}
  .side-icon{width:34px;height:34px;border-radius:12px;display:grid;place-items:center;background:rgba(255,255,255,.055);border:1px solid rgba(255,255,255,.08);color:var(--dash-cyan);box-shadow:inset 0 1px 0 rgba(255,255,255,.05)}
  ...
  .topbar{position:sticky;top:18px;z-index:10;border-radius:22px!important;padding:18px 20px!important;background:linear-gradient(180deg,color-mix(in srgb,var(--dash-panel) 92%,transparent),color-mix(in srgb,var(--dash-bg) 72%,transparent))!important;border:1px solid var(--dash-border)!important;box-shadow:var(--dash-shadow),var(--dash-glow)!important;backdrop-filter:blur(18px)}
  .title{font-size:34px!important;font-weight:900!important;text-shadow:0 0 26px rgba(139,92,246,.20)}
  .card,.item,.page-hero,.stat-tile,.module-option,.list-btn,details.acc,.preview-shell,.preview-embed{border-radius:18px!important;background:linear-gradient(180deg,var(--dash-panel),color-mix(in srgb,var(--dash-bg) 74%,transparent))!important;border:1px solid var(--dash-border)!important;box-shadow:0 20px 60px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.06)!important}
  .card:hover,.item:hover,.stat-tile:hover,.module-option:hover,.list-btn:hover{transform:translateY(-3px)!important;border-color:var(--dash-border-strong)!important;box-shadow:0 26px 70px rgba(0,0,0,.34),0 0 34px color-mix(in srgb,var(--dash-purple) 14%,transparent)!important}
  h1,h2,h3,strong,.list-title,.controller-name,.preview-title{color:var(--dash-text)}
  .muted,.help,.list-meta,.nav-sub{color:var(--dash-muted)!important}
  label{color:var(--dash-muted)!important;font-weight:750!important}
  ...
  input,select,textarea,.cs-trigger,.ms-trigger{border-radius:13px!important;background:color-mix(in srgb,var(--dash-bg) 82%,black 8%)!important;border:1px solid var(--dash-border)!important;color:var(--dash-text)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.035)!important;min-height:44px}
  input::placeholder,textarea::placeholder{color:var(--dash-muted-2)!important}
  input:focus,select:focus,textarea:focus,.custom-select.open .cs-trigger,.role-ms.open .ms-trigger{border-color:var(--dash-border-strong)!important;box-shadow:0 0 0 3px color-mix(in srgb,var(--dash-purple) 18%,transparent),0 0 28px color-mix(in srgb,var(--dash-purple) 12%,transparent)!important}
  .cs-menu,.ms-menu,.topnav-menu{background:linear-gradient(180deg,var(--dash-panel-3),var(--dash-panel))!important;border-color:var(--dash-border)!important;box-shadow:0 24px 70px rgba(0,0,0,.42),0 0 32px color-mix(in srgb,var(--dash-purple) 12%,transparent)!important}
  .cs-opt,.ms-item,.topnav-item{background:color-mix(in srgb,var(--dash-panel-2) 88%,transparent)!important;border-color:transparent!important;color:var(--dash-text)!important}
  .cs-opt:hover,.ms-item:hover,.topnav-item:hover,.cs-opt.active{border-color:var(--dash-border-strong)!important;background:linear-gradient(135deg,color-mix(in srgb,var(--dash-purple) 18%,transparent),color-mix(in srgb,var(--dash-blue) 9%,transparent))!important}
  .btn{border-radius:14px!important;background:linear-gradient(135deg,var(--dash-purple),var(--dash-blue))!important;color:#fff!important;border:1px solid color-mix(in srgb,var(--dash-purple) 62%,white 12%)!important;box-shadow:0 15px 34px color-mix(in srgb,var(--dash-purple) 24%,transparent)!important}
  .btn:hover{transform:translateY(-2px)!important;box-shadow:0 18px 42px color-mix(in srgb,var(--dash-purple) 30%,transparent),0 0 28px color-mix(in srgb,var(--dash-cyan) 12%,transparent)!important}
  .btn-soft{border-radius:14px!important;background:rgba(255,255,255,.055)!important;border-color:var(--dash-border)!important;color:var(--dash-text)!important}
  .btn-soft:hover{background:linear-gradient(135deg,color-mix(in srgb,var(--dash-purple) 14%,transparent),rgba(255,255,255,.06))!important;border-color:var(--dash-border-strong)!important;transform:translateY(-2px)!important}
  .btn-danger{border-radius:14px!important;background:linear-gradient(135deg,var(--dash-red),#fb7185)!important;color:#fff!important;border-color:rgba(248,113,113,.42)!important}
  .pill{border-radius:999px!important;background:rgba(255,255,255,.06)!important;border-color:var(--dash-border)!important;color:var(--dash-text)!important}
  .pill.ok{color:#dfffee!important;border-color:rgba(34,197,94,.32)!important;background:rgba(34,197,94,.12)!important}.pill.warn{color:#fff7d6!important;border-color:rgba(245,158,11,.38)!important;background:rgba(245,158,11,.14)!important}.pill.danger{color:#ffe4e6!important;border-color:rgba(239,68,68,.38)!important;background:rgba(239,68,68,.14)!important}
  .module-options{grid-template-columns:repeat(auto-fit,minmax(260px,1fr))!important;gap:16px!important}.module-option{min-height:142px!important;position:relative;overflow:hidden}.module-option:before{content:"";position:absolute;inset:auto -20% -45% -20%;height:78%;background:radial-gradient(circle,color-mix(in srgb,var(--dash-purple) 22%,transparent),transparent 60%);pointer-events:none}.module-option > *{position:relative}.module-option strong{font-size:17px!important}.module-option .pill{background:rgba(34,197,94,.10)!important;border-color:rgba(34,197,94,.28)!important}
  .module-editor-head{border-radius:18px!important;background:linear-gradient(135deg,color-mix(in srgb,var(--dash-purple) 16%,transparent),color-mix(in srgb,var(--dash-blue) 8%,transparent))!important;border:1px solid var(--dash-border)!important;padding:16px!important;margin-bottom:14px!important}
  .module-editor-title,.page-kicker{color:var(--dash-cyan)!important;letter-spacing:.13em!important;text-transform:uppercase!important;font-weight:900!important}
  ...
  /* Lumen premium blue/cyan dashboard system */
  :root{
    --lumen-blue:#3B82F6;
    --lumen-sky:#60A5FA;
    --lumen-cyan:#6DD5FA;
    --lumen-navy:#07111F;
    --lumen-midnight:#0B1629;
    --lumen-card:rgba(10,22,41,.72);
    --lumen-card-2:rgba(13,29,52,.78);
    --lumen-border:rgba(134,184,255,.16);
    --lumen-border-hot:rgba(109,213,250,.42);
    --lumen-text:#F8FBFF;
    --lumen-muted:rgba(220,232,255,.70);
    --lumen-soft:rgba(220,232,255,.48);
    --lumen-green:#3EE58F;
    --lumen-amber:#F5B84B;
    --lumen-red:#F05263;
    --lumen-shadow:0 18px 54px rgba(0,0,0,.34);
    --lumen-glow:0 0 44px rgba(59,130,246,.16);
    --dash-bg:var(--lumen-navy);
    --dash-bg-2:var(--lumen-midnight);
    --dash-panel:var(--lumen-card);
    --dash-panel-2:var(--lumen-card-2);
    --dash-panel-3:rgba(17,38,67,.92);
    --dash-border:var(--lumen-border);
    --dash-border-strong:var(--lumen-border-hot);
    --dash-purple:var(--lumen-blue);
    --dash-blue:var(--lumen-sky);
    --dash-cyan:var(--lumen-cyan);
    --dash-text:var(--lumen-text);
    --dash-muted:var(--lumen-muted);
    --dash-muted-2:var(--lumen-soft);
    --dash-shadow:var(--lumen-shadow);
    --dash-glow:var(--lumen-glow);
    --ac:var(--lumen-blue);
    --ac-soft:var(--lumen-cyan);
    --tx:var(--lumen-text);
    --mt:var(--lumen-muted);
    --bd:var(--lumen-border);
  }
  ...
  body[data-theme],body:not([data-theme]){
    background:
      ...
      radial-gradient(760px 500px at 96% 8%,rgba(109,213,250,.16),transparent 60%),
      radial-gradient(800px 560px at 75% 112%,rgba(62,229,143,.06),transparent 62%),
      linear-gradient(145deg,#050B15 0%,var(--lumen-navy) 46%,var(--lumen-midnight) 100%)!important;
    min-height:100vh;
  }
  body:before{content:"";position:fixed;inset:0;z-index:-2;pointer-events:none;background-image:linear-gradient(rgba(109,213,250,.045) 1px,transparent 1px),linear-gradient(90deg,rgba(109,213,250,.045) 1px,transparent 1px);background-size:48px 48px;mask-image:radial-gradient(circle at 50% 10%,#000 0%,transparent 72%);opacity:.55}
  body:after{content:"";position:fixed;inset:0;z-index:-1;pointer-events:none;background:radial-gradient(circle at 18% 22%,rgba(109,213,250,.12),transparent 4px),radial-gradient(circle at 72% 18%,rgba(96,165,250,.11),transparent 3px),radial-gradient(circle at 86% 58%,rgba(109,213,250,.08),transparent 3px),radial-gradient(circle at 38% 78%,rgba(96,165,250,.08),transparent 3px);background-size:420px 360px;animation:lumenParticles 18s linear infinite;opacity:.7}
  @keyframes lumenParticles{from{transform:translateY(0)}to{transform:translateY(-90px)}}
  .layout{grid-template-columns:292px minmax(0,1fr)!important}
  .app-sidebar{padding:24px 18px!important;background:linear-gradient(180deg,rgba(7,17,31,.82),rgba(6,13,25,.64))!important;border-right:1px solid var(--lumen-border)!important;box-shadow:16px 0 48px rgba(0,0,0,.26)!important;backdrop-filter:blur(22px)!important}
  .sidebar-brand{padding:8px 8px 18px!important;border-bottom:1px solid rgba(134,184,255,.12)!important}
  .sidebar-logo{width:50px!important;height:50px!important;border-radius:16px!important;background:linear-gradient(135deg,rgba(59,130,246,.32),rgba(109,213,250,.16))!important;border-color:rgba(109,213,250,.34)!important;box-shadow:0 0 34px rgba(59,130,246,.20)!important}
  .sidebar-brand strong{font-size:17px!important;font-weight:850!important}.sidebar-brand small,.side-section-title,.sidebar-profile small{color:var(--lumen-soft)!important}
  .side-link{border-radius:14px!important;padding:12px 13px!important;color:rgba(220,232,255,.68)!important}
  .side-link:hover{background:rgba(96,165,250,.075)!important;border-color:rgba(109,213,250,.20)!important;color:#fff!important;transform:translateX(3px)!important}
  .side-link.active{background:linear-gradient(135deg,rgba(59,130,246,.24),rgba(109,213,250,.13))!important;border-color:rgba(109,213,250,.42)!important;box-shadow:0 0 0 1px rgba(109,213,250,.08),0 18px 40px rgba(59,130,246,.14)!important}
  .side-icon,.nav-kicker{background:rgba(96,165,250,.10)!important;border-color:rgba(109,213,250,.18)!important;color:var(--lumen-cyan)!important}
  .premium-card{background:linear-gradient(145deg,rgba(59,130,246,.24),rgba(109,213,250,.10))!important;border-color:rgba(109,213,250,.30)!important;box-shadow:0 0 42px rgba(59,130,246,.13)!important}
  .premium-card a{color:#fff!important}
  .main{padding:30px 38px 52px!important}
  .topbar{top:18px!important;border-radius:22px!important;background:linear-gradient(180deg,rgba(12,28,50,.70),rgba(7,17,31,.50))!important;border:1px solid rgba(134,184,255,.16)!important;box-shadow:0 18px 54px rgba(0,0,0,.26),0 0 40px rgba(59,130,246,.08)!important;backdrop-filter:blur(24px)!important}
  .brand-mini{border-radius:15px!important;background:rgba(96,165,250,.10)!important;border:1px solid rgba(109,213,250,.20)!important;box-shadow:0 0 24px rgba(59,130,246,.10)!important}
  .title{font-size:clamp(28px,3.4vw,38px)!important;font-weight:900!important;letter-spacing:0!important;text-shadow:none!important}
  .title,.page-hero h3,.welcome h3{background:linear-gradient(135deg,#fff 35%,var(--lumen-cyan));-webkit-background-clip:text;background-clip:text;color:transparent!important}
  .muted,.help,.list-meta,.nav-sub,.page-hero p{color:var(--lumen-muted)!important;line-height:1.65!important}
  .page-shell{gap:20px!important;animation:lumenIn .24s ease both}
  @keyframes lumenIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}
  .card,.item,.page-hero,.stat-tile,.module-option,.list-btn,details.acc,.pricing-card,.pricing-cta,.pricing-table,.preview-shell,.preview-embed,.controller-card,.server-card{
    border-radius:18px!important;
    background:linear-gradient(180deg,rgba(13,29,52,.78),rgba(8,19,35,.70))!important;
    border:1px solid var(--lumen-border)!important;
    box-shadow:var(--lumen-shadow),inset 0 1px 0 rgba(255,255,255,.045)!important;
    backdrop-filter:blur(18px)!important;
  }
  .card:hover,.item:hover,.stat-tile:hover,.module-option:hover,.list-btn:hover,.pricing-card:hover{
    transform:translateY(-3px)!important;
    border-color:var(--lumen-border-hot)!important;
    background:linear-gradient(180deg,rgba(15,36,64,.84),rgba(9,23,42,.76))!important;
    box-shadow:0 22px 62px rgba(0,0,0,.30),0 0 34px rgba(59,130,246,.11)!important;
  }
  .page-hero{position:relative;overflow:hidden;padding:28px!important}
  .page-hero:before,.welcome:before{content:"";position:absolute;inset:auto -10% -55% 10%;height:90%;background:radial-gradient(circle,rgba(109,213,250,.18),transparent 64%);pointer-events:none}
  .page-hero > *,.welcome > *{position:relative}
  .page-kicker,.module-editor-title,.pricing-kicker{color:var(--lumen-cyan)!important;letter-spacing:.14em!important;text-transform:uppercase!important;font-weight:900!important}
  .stat-strip{gap:18px!important}.stat-tile{min-height:138px!important;padding:22px!important}
  .stat-tile strong{font-size:clamp(30px,3.4vw,42px)!important;background:linear-gradient(135deg,#fff,var(--lumen-sky));-webkit-background-clip:text;background-clip:text;color:transparent!important}
  .stat-tile:before{background:radial-gradient(circle,rgba(109,213,250,.20),transparent 62%)!important}
  .btn,.invite-action{background:linear-gradient(135deg,var(--lumen-blue),var(--lumen-cyan))!important;border:1px solid rgba(174,221,255,.34)!important;color:#fff!important;border-radius:14px!important;font-weight:850!important;box-shadow:0 14px 34px rgba(59,130,246,.24),0 0 24px rgba(109,213,250,.08)!important}
  .btn:hover,.invite-action:hover{transform:translateY(-2px)!important;filter:brightness(1.05)!important;box-shadow:0 18px 42px rgba(59,130,246,.30),0 0 34px rgba(109,213,250,.14)!important}
  .btn-soft,.topnav-btn,.server-icon-btn{background:rgba(96,165,250,.075)!important;border:1px solid rgba(134,184,255,.15)!important;color:var(--lumen-text)!important;border-radius:14px!important}
  .btn-soft:hover,.topnav-btn:hover{background:rgba(96,165,250,.12)!important;border-color:rgba(109,213,250,.32)!important;transform:translateY(-2px)!important}
  .btn-danger{background:rgba(240,82,99,.10)!important;border:1px solid rgba(240,82,99,.38)!important;color:#ffe9ed!important;box-shadow:0 0 24px rgba(240,82,99,.10)!important}
  input,select,textarea,.cs-trigger,.ms-trigger{min-height:46px!important;border-radius:14px!important;background:rgba(5,13,25,.76)!important;border:1px solid rgba(134,184,255,.16)!important;color:var(--lumen-text)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.035)!important}
  input:focus,select:focus,textarea:focus,.custom-select.open .cs-trigger,.role-ms.open .ms-trigger{border-color:rgba(109,213,250,.55)!important;box-shadow:0 0 0 3px rgba(59,130,246,.16),0 0 30px rgba(109,213,250,.10)!important}
  input::placeholder,textarea::placeholder{color:rgba(220,232,255,.42)!important}
  .cs-menu,.ms-menu,.topnav-menu{background:linear-gradient(180deg,rgba(13,29,52,.98),rgba(7,17,31,.96))!important;border:1px solid rgba(134,184,255,.18)!important;border-radius:16px!important;box-shadow:0 24px 70px rgba(0,0,0,.42),0 0 28px rgba(59,130,246,.08)!important;backdrop-filter:blur(22px)!important}
  .cs-opt,.ms-item,.topnav-item{background:rgba(96,165,250,.06)!important;border-color:transparent!important;color:var(--lumen-text)!important;border-radius:12px!important}
  .cs-opt:hover,.ms-item:hover,.topnav-item:hover,.cs-opt.active{background:linear-gradient(135deg,rgba(59,130,246,.18),rgba(109,213,250,.09))!important;border-color:rgba(109,213,250,.28)!important}
  .pill,.role,.ms-chip{background:rgba(96,165,250,.08)!important;border:1px solid rgba(134,184,255,.16)!important;color:var(--lumen-text)!important;border-radius:999px!important}
  .pill.ok{background:rgba(62,229,143,.10)!important;border-color:rgba(62,229,143,.30)!important;color:#dfffee!important}.pill.warn{background:rgba(245,184,75,.12)!important;border-color:rgba(245,184,75,.32)!important;color:#fff3d1!important}.pill.danger{background:rgba(240,82,99,.12)!important;border-color:rgba(240,82,99,.32)!important;color:#ffe3e8!important}
  .status-pill{background:rgba(62,229,143,.10)!important;border-color:rgba(62,229,143,.24)!important}.status-dot{background:var(--lumen-green)!important;box-shadow:0 0 14px rgba(62,229,143,.85)!important}
  .module-options{grid-template-columns:repeat(auto-fit,minmax(280px,1fr))!important;gap:18px!important}.module-option{min-height:150px!important}.module-option:after{content:"->";position:absolute;right:18px;bottom:16px;color:var(--lumen-cyan);opacity:0;transform:translateX(-4px);transition:all .18s ease}.module-option:hover:after{opacity:1;transform:none}
  .lumen-analytics{display:grid;grid-template-columns:minmax(0,1.45fr) minmax(280px,.75fr);gap:20px}
  .lumen-chart-card canvas{width:100%;height:260px;display:block}
  .lumen-card-head{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;margin-bottom:16px}
  .lumen-tabs{display:flex;gap:8px;flex-wrap:wrap}.lumen-tabs .btn-soft{width:auto;padding:8px 11px}
  .lumen-status-list{display:grid;gap:10px}.lumen-status-row{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px;border-radius:14px;border:1px solid rgba(134,184,255,.12);background:rgba(96,165,250,.05)}
  input,select,textarea,button{font-family:Inter,"Readex Pro","Segoe UI",system-ui,-apple-system,sans-serif!important;letter-spacing:0}
  .level-up-card{min-height:520px;display:grid;place-items:center;text-align:center;overflow:hidden;position:relative}.slot-window{height:86px;overflow:hidden;margin:22px auto;width:min(620px,100%);border-radius:18px;border:1px solid rgba(134,184,255,.18);background:rgba(255,255,255,.05);box-shadow:inset 0 1px 0 rgba(255,255,255,.08)}.slot-reel{display:grid;animation:slotRise 4.2s cubic-bezier(.2,.8,.2,1) forwards}.slot-reel div{height:86px;display:grid;place-items:center;font-size:clamp(20px,3.8vw,36px);font-weight:900}.level-up-glow{position:absolute;inset:auto 12% -160px;height:280px;background:radial-gradient(circle,rgba(103,232,249,.28),transparent 68%);pointer-events:none}@keyframes slotRise{0%{transform:translateY(0)}22%{transform:translateY(-86px)}46%{transform:translateY(-172px)}70%{transform:translateY(-258px)}100%{transform:translateY(-344px)}}
...
<body>
 ...
 <div class="layout"><main class="main"><div class="topbar"><div class="topbar-left"><a class="brand-mini" href="/" title="Landing page"><img src="/assets/sync.png" alt="Tickets Dashboard" /></a><div class="titles"><h2 id="pageTitle" class="title">${pageTitle}</h2><div class="muted" id="pageHint">${pageDescriptionForPath(currentPath)}</div></div></div><div class="topbar-right"><a class="btn-soft server-icon-btn" href="/dashboard" title="Servers" aria-label="Servers"><span class="btn-icon">${dashboardIcon('servers')}</span></a><div id="themeNav" class="topnav"><button id="themeBtn" class="btn-soft topnav-btn" type="button"><span id="themeLabel">Theme</span><span class="chev">v</span></button><div class="topnav-menu" role="menu"><button type="button" class="topnav-item" data-theme-item="site">Default</button><button type="button" class="topnav-item" data-theme-item="dark">Dark</button><button type="button" class="topnav-item" data-theme-item="light">Light</button><button type="button" class="topnav-item" data-theme-item="ocean">Ocean</button><button type="button" class="topnav-item" data-theme-item="sunset">Sunset</button><button type="button" class="topnav-item" data-theme-item="diamond">Diamond</button></div></div><button id="refreshStateBtn" class="btn" style="padding:10px 16px"><span class="btn-icon">${dashboardIcon('restart')}</span><span>Refresh</span></button></div></div><div id="announcementBar"></div><div id="notice" class="notice"></div><section id="app"></section></main></div>
<script>${pricingLocalizationScript()}</script>
<script>
...
const app=document.getElementById('app'),notice=document.getElementById('notice'),auth=document.getElementById('auth'),authDiscord=document.getElementById('authDiscord'),authToken=document.getElementById('authToken'),authMsg=document.getElementById('authMsg');
 ...
 const applyTheme=(value)=>{const allowed=['site','dark','light','ocean','sunset','diamond','hacker'];const next=allowed.includes(String(value||'').toLowerCase())?String(value||'site').toLowerCase():'site';document.body.dataset.theme=next;document.body.dataset.hackerUnlocked=next==='hacker'?'true':'false';try{localStorage.setItem(themeKey,next)}catch{};const label=document.getElementById('themeLabel');if(label)label.textContent='Theme: '+next.charAt(0).toUpperCase()+next.slice(1);document.querySelectorAll('[data-theme-item]').forEach(btn=>btn.classList.toggle('active',btn.getAttribute('data-theme-item')===next));};
 const forceSiteTheme=()=>applyTheme((()=>{try{return localStorage.getItem(themeKey)||'site'}catch{return 'site'}})());
 forceSiteTheme();
  let state=null;
...
function renderAvailability(){const types=(state.ticketTypes||[]);const byKey=new Map((state.availability||[]).map(v=>[v.key,v]));const opts='<option value=\"auto\">Automatic</option><option value=\"available\">Available</option><option value=\"increased_volume\">Increased Volume</option><option value=\"reduced_assistance\">Reduced Assistance</option>';const rows=types.map(t=>{const key=String(t.name||'').trim().toLowerCase();const info=byKey.get(key)||{status:'available',count:0,source:'automatic'};const manual=info.manualStatus||'auto';return '<div class=\"item\"><div class=\"item-top\"><strong>'+esc(t.name)+'</strong><span>'+availabilityBadge(info)+'</span></div><div class="row" style="margin-top:8px"><div><label>Override</label><select class="availSelect" data-name="'+esc(t.name)+'\">'+opts.replace('value=\"'+esc(manual)+'\"','value=\"'+esc(manual)+'\" selected')+'</select></div><div><label>Automatic</label><div class="muted">'+availabilityLabel(info.automaticStatus||'available')+'</div></div></div></div>'}).join('');return '<div class="grid"><div class="card"><h3>Ticket Availability</h3><p class="muted">Overrides apply per ticket type. Automatic mode uses active ticket thresholds.</p></div><div class="card"><h3>Per Ticket Type</h3><div class="list\">'+(rows||'<span class="muted">No ticket types configured yet.</span>')+'</div></div></div>'}
  function renderTypes(){
   ...
   const questionLimit=(state.aiAccess&&state.aiAccess.hasAccess)?5:1;
  ...
  const list=types
   ...
   .map(t=>{
     ...
     return '<button type="button" class="list-btn ttPick '+(active?'active':'')+'" data-name="'+esc(t.name)+'">'+
       ...
       '<div class="list-meta">'+esc(cat)+' - '+(t.requireReason===false?'No reason':'Reason required')+' - '+(t.allowAttachments===false?'No files':'Files ok')+' - '+(t.aiEnabled===false?'AI off':'AI ok')+'</div>'+
     '</button>';
    ...
    '<div class="card">'+
      ...
      '<div class="row">'+
        ...
        '<div>'+
          ...
          '<div class="help">Channel tokens: <code>{number}</code>, <code>{userId}</code>, <code>{username}</code>, <code>{priority}</code>, <code>{type}</code>, <code>{suffix}</code>.</div>'+
        '</div>'+
      '</div>'+

      '<label>Common Channel Formats</label>'+
      '<select id="ttFormatPreset">'+
        '<option value="">Choose a format...</option>'+
        '<option value="ticket-{number}">ticket-1, ticket-2 (ticket IDs)</option>'+
      ...
      '<div class="row" style="grid-template-columns:1fr 1fr">'+
        '<div style="display:grid;gap:6px;margin-top:6px">'+
         ...
         '</label>'+
         '<label class="checkbox-wrapper">'+
          '<input type="checkbox" id="ttAiEnabled" checked />'+
          '<span class="checkmark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg></span>'+
          '<span class="label">Allow AI on this ticket type</span>'+
         '</label>'+
        '</div>'+
      '</div>'+
...
function renderPricing(){const priceHtml=plan=>Number.isFinite(plan.gbp)?'<span class="localized-price" data-price-gbp="'+plan.gbp.toFixed(2)+'">&pound;'+plan.gbp.toFixed(2)+'</span><span class="plan-once">one-time payment</span>':esc(plan.price);const plans=[{name:'Free',price:'Free',description:'Core support tools for getting started',features:['Unlimited tickets','Ticket panels','Logs and transcripts','Dashboard access'],active:true},{name:'Plus',price:'&pound;8.99',gbp:8.99,description:'Better visibility for growing teams',features:['Statistics','Staff activity tracking','Priority support','Everything in Free'],featured:true},{name:'Pro',price:'&pound;14.99',gbp:14.99,description:'Automation for busier support teams',features:['AI moderation','Advanced analytics','Higher automation limits','Everything in Plus']},{name:'Enterprise',price:'Custom',description:'Custom branded bot and guided setup',features:['Custom bot runtime','Developer Portal guidance','Webhook monitoring','Everything in Pro']}];const rows=[['Tickets','Yes','Yes','Yes','Yes'],['Panels','Yes','Yes','Yes','Yes'],['Logs and Transcripts','Yes','Yes','Yes','Yes'],['Statistics','No','Yes','Yes','Yes'],['AI Moderation','No','No','Yes','Yes'],['Custom Branded Bot','No','No','No','Yes'],['Priority Support','No','Yes','Yes','Yes']];const faqs=[{q:'Can I start on Free and upgrade later?','a':'Yes. Free is available immediately and upgrades keep your configuration.'},{q:'Are Plus and Pro monthly?','a':'No. Plus and Pro are one-time payments, shown in your local currency when possible.'},{q:'What does Pro add?','a':'Pro adds AI moderation and advanced automation for busier support teams.'},{q:'What is Enterprise?','a':'Enterprise is the custom plan with branded bot runtime setup, monitoring, and guided Developer Portal configuration.'}];return '<div class="page-shell pricing-page">'+
  ...
  '</div>'}
function renderUpgrade(){const support=${JSON.stringify(String(process.env.SUPPORT_SERVER_URL || process.env.DISCORD_SUPPORT_URL || '').trim())};return '<section class="card level-up-card"><div class="level-up-glow"></div><div style="position:relative;z-index:1;max-width:760px"><div class="page-kicker">Upgrade preview</div><h3 style="font-size:clamp(30px,5vw,56px);margin:0">You are levelling up your support.</h3><div class="slot-window"><div class="slot-reel"><div>you are levelling up your support!</div><div>prepping your agents...</div><div>storing tickets...</div><div>checking support systems...</div><div>here we go!</div></div></div><p class="muted">We will open the support server in a moment so you can choose Plus, Pro, or a custom branded bot setup.</p><div class="row" style="justify-content:center;margin-top:18px"><button id="upgradeGoNow" class="btn primary" type="button">Open support now</button><a class="btn-soft" href="/dashboard/pricing">Compare plans</a></div><span id="upgradeSupportUrl" data-url="'+esc(support)+'"></span></div></section>'}function renderTutorials(){
...
function setupModuleDrilldown(){
 if(['/overview','/tutorials','/documentation','/dashboard/pricing','/dashboard/upgrade'].includes(currentPath))return;
 ...
 const openPanel=(panel,title)=>{
  ...
  if(!panel.querySelector(':scope > .module-editor-head')){
   ...
   head.className='module-editor-head';
    head.innerHTML='<div><div class="module-editor-title">Dashboard / Modules</div><h3 style="margin:4px 0 0">'+esc(title)+'</h3></div><button type="button" class="btn-soft moduleBack" style="width:auto">Close</button>';
...
function wire(){
 ...
 window.__navWired=true;
  ...
  const themeBtn=document.getElementById('themeBtn');
    ...
    const pageDescForPath=(p)=>({ '/overview':'A cleaner snapshot of ticket activity, queue health, and the most common next actions.','/settings':'Core server configuration, routing, and system behavior in one place.','/availability':'Adjust queue expectations per ticket type without digging through commands.','/tutorials':'Guides, walkthroughs, and internal onboarding material for your staff.','/commands/ticket-types':'Shape each ticket flow, assign support coverage, and keep categories tidy.','/panels':'Design, save, and publish channel-specific ticket panels.','/commands/tag':'Store reusable answers and keep repeat support responses consistent.','/tickets':'Review active conversations, add notes, and handle escalations quickly.','/transcripts':'Browse saved transcripts and archive history without leaving the dashboard.','/commands/feedback':'Control where feedback lands and how the flow is presented.','/statistics':'Track recent performance, close reasons, and staff activity trends.','/embed-editor':'Customize server branding and reusable bot message templates.','/dashboard/pricing':'Compare plans and see what is available for this server.','/dashboard/upgrade':'Upgrade to Plus or Pro as one-time purchases, or contact sales for Enterprise.','/documentation':'Reference placeholders, templates, and dashboard usage notes.'}[p]||'Manage this part of the dashboard with a simpler, more focused layout.');
     ...
     const allowedPages=()=>{const access=(state&&state.access)||{};const plan=(state&&state.aiAccess)||{};const set=new Set(['/documentation','/tutorials','/dashboard/pricing','/dashboard/upgrade','/privacy','/terms']);if(access.isOwner||access.canFullDashboard){['/overview','/settings','/availability','/commands/ticket-types','/panels','/commands/tag','/tickets','/transcripts','/commands/feedback','/dashboard/pricing','/dashboard/upgrade'].forEach(p=>set.add(p));if(plan.isPlusOrHigher)set.add('/statistics');set.add('/embed-editor');return set}if(access.isManager||access.isStaff||access.canManageSettings||access.canViewTickets)set.add('/overview');if(access.canManageTicketTypes){set.add('/settings');set.add('/commands/ticket-types');set.add('/panels');set.add('/embed-editor')}if(plan.isPlusOrHigher&&access.canManageTicketTypes)set.add('/statistics');if(access.canManageAvailability)set.add('/availability');if(access.canViewTickets||access.canManageEscalations)set.add('/tickets');if(access.canViewTranscripts)set.add('/transcripts');return set};
     forceSiteTheme();
     if(themeNav&&themeBtn){themeBtn.onclick=(ev)=>{ev.stopPropagation();const next=!themeNav.classList.contains('open');closePickers();closeTopNav();if(next)themeNav.classList.add('open');else themeNav.classList.remove('open')}};
     document.querySelectorAll('[data-theme-item]').forEach(btn=>{btn.onclick=(ev)=>{ev.stopPropagation();applyTheme(btn.getAttribute('data-theme-item')||'site');if(themeNav)themeNav.classList.remove('open')}});
   ...
   topNavItems.forEach(btn=>{const v=btn.getAttribute('data-value')||'';btn.style.display=allowedPages().has(v)?'flex':'none';btn.onclick=()=>{navigate(v)}});
    ...
    syncNav();syncGroups();
    window.addEventListener('keydown',(ev)=>{if(ev.key==='Escape'){closeMenu();closePickers();closeTopNav();if(themeNav)themeNav.classList.remove('open');}}, { passive: true });
    document.addEventListener('click',()=>{if(themeNav)themeNav.classList.remove('open')});
 ...
 document.querySelectorAll('.removeExclusion').forEach(btn=>btn.onclick=async()=>{try{await api('/api/exclusion/delete',{method:'POST',body:JSON.stringify({guildId:state.guildId,userId:btn.dataset.user||''})});note('Exclusion removed.','ok');await boot()}catch(e){note(e.message,'danger')}});
  const aiUpsell=document.getElementById('aiUpsell');if(aiUpsell)aiUpsell.onclick=()=>{window.location='/dashboard/pricing'};
 ...
 const upgradeGoNow=document.getElementById('upgradeGoNow');if(upgradeGoNow){const supportEl=document.getElementById('upgradeSupportUrl');const supportUrl=String((supportEl&&supportEl.dataset&&supportEl.dataset.url)||'').trim();const go=()=>{const lower=supportUrl.toLowerCase();if(lower.startsWith('https://')||lower.startsWith('http://'))window.location.href=supportUrl;else note('Support server URL is not configured yet.','warn')};upgradeGoNow.onclick=go;setTimeout(go,4600);};
...
const resetTeam=document.getElementById('resetTeam');if(resetTeam)resetTeam.onclick=()=>{stName.value='';stEmoji.value='';setRoleSelection('stRoles',[])};
  ...
  const resetType=document.getElementById('resetType');if(resetType)resetType.onclick=()=>{['ttName','ttEmoji','ttFormat','ttAliases','ttOpenQuestions','ttOpenTitle','ttOpenDescription'].forEach(id=>{const el=document.getElementById(id);if(el)el.value=''});const catEl=document.getElementById('ttCategory');if(catEl)catEl.value='';const catLabel=document.getElementById('ttCategoryLabel');if(catLabel)catLabel.textContent=categoryLabel('', 'Use default ticket category');ttColor.value='#5865F2';ttRequireReason.checked=true;ttAllowFiles.checked=true;const ttAiEnabled=document.getElementById('ttAiEnabled');if(ttAiEnabled)ttAiEnabled.checked=true;renderQuestionBuilder(['Describe your issue and what you need from us.']);setRoleSelection('ttRoles',[])};
...
function renderOverview(){
 ...
 const tickets=Array.isArray(state&&state.tickets)?state.tickets.slice(0,5):[];
 const modulesReady=[
  ['Tickets',Number(totals.activeTickets||0)+' active','/tickets'],
  ['Panels',state&&state.guildConfigSummary&&state.guildConfigSummary.panelConfig?'Configured':'Ready','/panels'],
  ['AI',state&&state.aiAccess&&state.aiAccess.hasAccess?'Available':'Free','/settings'],
  ['Logs',state&&state.transcripts&&state.transcripts.length?String(state.transcripts.length)+' saved':'No entries','/transcripts']
 ];
 ...
 return '<div class="page-shell">'+
  ...
  '</div>'+
  '<div class="lumen-analytics">'+
   '<div class="card lumen-chart-card"><div class="lumen-card-head"><div><h3 style="margin:0">Activity Overview</h3><div class="muted">Claimed and closed ticket activity over recent days.</div></div><div class="lumen-tabs"><button type="button" class="btn-soft statsView" data-chart="bar">Bars</button><button type="button" class="btn-soft statsView" data-chart="line">Line</button><button type="button" class="btn-soft statsView" data-chart="area">Area</button></div></div><canvas id="statsChart"></canvas></div>'+
   '<div class="card"><div class="lumen-card-head"><div><h3 style="margin:0">System Status</h3><div class="muted">Operational readiness for this server.</div></div><span class="pill ok">Online</span></div><div class="lumen-status-list">'+
    modulesReady.map(m=>'<button type="button" class="lumen-status-row btn-soft qa" data-go="'+esc(m[2])+'"><span><strong>'+esc(m[0])+'</strong><div class="muted">'+esc(m[1])+'</div></span><span class="pill">Open</span></button>').join('')+
   '</div></div>'+
  '</div>'+
  '<div class="grid">'+
  ...
  '<div class="card"><h3>Recent Events</h3><div class="list" style="margin-top:10px">'+
   (tickets.length?tickets.map(t=>'<div class="item"><div class="item-top"><strong>#'+esc(t.channelName||t.channelId||'ticket')+'</strong><span class="pill">'+esc(t.ticketType||'Ticket')+'</span></div><div class="muted">'+esc(t.createdBy?'Opened by '+t.createdBy:'Open ticket')+(t.claimedBy?' - claimed by '+esc(t.claimedBy):'')+'</div></div>').join(''):'<div class="muted">No recent ticket events yet.</div>')+
  '</div></div>'+
    ...
    dashboardServer = http.createServer(async (req, res) => {
        try {
            ...
            if (pathname.startsWith('/assets/')) {
                ...
                streamFileToResponse(res, resolved);
                return;
            }
            ...
            const pages = new Set(['/overview', '/settings', '/availability', '/tutorials', '/commands/ticket-types', '/panels', '/commands/tag', '/tickets', '/transcripts', '/commands/feedback', '/statistics', '/embed-editor', '/dashboard/pricing', '/dashboard/upgrade', '/documentation', '/privacy', '/terms']);
